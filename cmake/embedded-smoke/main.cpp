...
byte_stream::byte_stream_errc encode(
    byte_stream::byte_writer& writer, uint16_t sequence, uint32_t area_code) noexcept {
    auto status = writer.write(sequence);
    if (status != byte_stream::byte_stream_errc::ok) return status;
    return writer.write(area_code, 3);
}

byte_stream::byte_stream_errc decode(
    byte_stream::byte_reader& reader, uint16_t& sequence, uint32_t& area_code) noexcept {
    auto status = reader.read(sequence);
    if (status != byte_stream::byte_stream_errc::ok) return status;
    return reader.read(area_code, 3);
}
...
int main() {
    ...
    if (decode(reader, sequence, area_code) != byte_stream::byte_stream_errc::ok) return 2;
    return sequence == 0x1234 && area_code == 0x00A1B2C3 ? 0 : 3;
}
